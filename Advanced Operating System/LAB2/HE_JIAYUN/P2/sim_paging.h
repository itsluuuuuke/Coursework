...
typedef struct
{
    ...
    int numframes;
    sframe * frt;
    int listfree;          // Stores the number of the last frame of the list
    int listoccupied;      // Only for FIFO and FIFO 2nd ch.

    // Trace data

    //new added fields
    unsigned timestamp_MIN;// Stores the value of the minimum timestamp
    unsigned timestamp_MAX;// Stores the value of the maximum timestamp
    int pag_with_MIN_timestamp; //Stores the page with minimum timestamp
    int pag_with_MAX_timestamp; //Stores the page with maximum timestamp
