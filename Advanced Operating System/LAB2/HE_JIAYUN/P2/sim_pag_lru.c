...
{
    int frame,victim,i; //victim is the #page that will be replaced
	unsigned local_min = UINT_MAX; //To store the minimum timestamp
   	
	//traverse all the frames to search for the page
	...
	frame = S->pgt[victim].frame; //marked the victim frame
...
void print_replacement_report (ssystem * S)
{
	...
	//Search and stored the smallest timestamp
